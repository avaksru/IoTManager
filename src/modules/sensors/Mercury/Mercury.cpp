...
#define MODBUS_RX_PIN 22 // Rx pin
#define MODBUS_TX_PIN 15 // Tx pin
#define MODBUS_DIR_PIN  23 // Dir pin
#define MODBUS_SERIAL_BAUD 9600

int8_t _dir = MODBUS_DIR_PIN;//new
...
public:
    MercuryNode(String parameters) : IoTItem(parameters)
    {
        ...
        SerialPrint("I", "MercuryNode", String("Добавлен узел/токен: ") + getID().c_str() + " - " + String(_token));
    }
...
public:
    MercuryClient(String parameters) : IoTItem(parameters)
    {
        ...
        _dir = jsonReadInt(parameters, "DIR_PIN");
 ...
 ///pinMode(LED_BUILTIN, OUTPUT);//new
        ...
        if (_debug)
        {
            SerialPrint("I", "MercuryClient", "baud: " + String(_baud) + ", protocol: " + String(protocol, HEX) + 
            ", RX: " + String(_rx) + ", TX: " + String(_tx) +  ", DIR: " + String(_dir));
        }
